...
class ServiceInstances extends CloudControllerBase {
  ...
  getInstancePermissions(guid) {
     ...
     * {@link https://apidocs.cloudfoundry.org/226/service_instances
          /list_all_service_bindings_for_the_service_instance.html}
     *
     * @param  {String} guid         [servicePlan guid]
     * @return {JSON}              [return a JSON response]
     */
  ...
  getInstanceRoutes(guid) {
     ...
     * {@link https://apidocs.cloudfoundry.org/226/service_instances/creating_a_service_instance.html}
     *
  ...
  remove(guid) {
     ...
     * {@link https://apidocs.cloudfoundry.org/226/service_instances/update_a_service_instance.html}
     *
     * @param  {String} serviceInstanceGuid     [service instance guid]
     * @param  {JSON} serviceInstanceOptions   [options to update the instance]
     * @return {JSON}              [information about the instance]
     */
  update(serviceInstanceGuid, serviceInstanceOptions) {

    const url = `${this.API_URL}/v2/service_instances/${serviceInstanceGuid}`;
    const options = {
      method: 'PUT',
      url: url,
      headers: {
